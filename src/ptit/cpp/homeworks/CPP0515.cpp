...
struct Student {
    ...
    void print() const {
        cout << id << " " << fullname << " " << classroom << " " << dob << " " << fixed << setprecision(2) << gpa <<
                endl;
    }
};
...
void quickSort(Student *arr, const int left, const int right) {
    if (left >= right) return;

    int tempL = left, tempR = right;
    const Student pivot = arr[(left + right) / 2];

    while (tempL <= tempR) {
        while (arr[tempL].gpa > pivot.gpa) tempL++;
        while (arr[tempR].gpa < pivot.gpa) tempR--;
